...
class CustomTokenizer():
    ...
    def replace_fig_to_num(self, text):
        ...
        for word in text.split():
            if '각형' in word or '면체' in word:
                word = word.translate(str.maketrans(number,'123456789'))
                if '십' in word:
                    idx = word.find('십')
                    pre = word[idx - 1].isdigit()
                    nex = word[idx + 1].isdigit()
                    if pre and nex:
                        word = word.replace(word[idx - 1:idx + 2], str(10 * int(word[idx - 1]) + int(word[idx + 1])))
                    elif pre:
                        word = word.replace('십', '0')
                    elif nex:
                        word = word.replace('십', '1')
                    else:
                        word = word.replace('십', '10')
            result.append(word)
        return ' '.join(result)
                ...
                for value in values:
                    if len(value) == 1:
                        if word == value:
                            ...
                            try:
                                if value not in self.komoran.nouns(word) or '보다' in word:
                                    continue
                            except:
                                continue# 배를 -> foo 를
                            key = get_key(value, data)
    ...
    def get_group_num(self, text):
        words = text.split()
        group_num=[]
        main_words=['더','까지','부터','앞', '뒤','마지막','가운데', '나중', '홀수', '짝수', '소수','평균','먼저','늦게','째','꼴찌',
                    '모서리','넓이','잘못', '가장','보다','합','차','곱','모두','적어도','순서','몫','나머지','오른쪽','왼쪽','씩',
                    '제대로','바르게','원래','반복','차례','소수점','중복','배']
        for idx, word in enumerate(words):
