...
def main():
    ...
    for elem in datasets['train']:
        src = elem['translation']['src']
        target = elem['translation']['target']

        special_tokens.update(set(target.split()))

    logger.info(f'len(special_tokens) = {len(special_tokens)}')
    ...
    if training_args.do_predict:
        ...
        if trainer.is_world_process_zero():
            # tokenizer.decode(predict_dataset[0]['input_ids'])
