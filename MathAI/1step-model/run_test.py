...
def calculate_corpus_bleu_score(args):
    test_file = f'test_out_beam_{args.beam_size}.json' if args.beam_search else 'test_out_greedy.json'
    with open(os.path.join(args.save_model_dir, test_file), 'r', encoding='utf-8') as f:
        data = json.load(f)
    refs, cands = [], []
    for elem in data:
        refs.append([elem['trg']])
        cands.append(elem['predicted'])
    return corpus_bleu(refs, cands)


def calculate_result_accurary(args):
    test_file = f'test_out_beam_{args.beam_size}.json' if args.beam_search else 'test_out_greedy.json'
    with open(os.path.join(args.save_model_dir, test_file), 'r', encoding='utf-8') as f:
        data = json.load(f)
    ...
    correct = 0
    for elem in data:
        if elem['trg'] == elem['predicted']:
            correct += 1
    return correct / total_count


def translate_Transformer(batch, vocabs, model, device, max_decode_len):
    '''
    ...
    trg_idx = [TRG_VOCAB.stoi[init_token]]
    ...
    trg_tokens = [TRG_VOCAB.itos[i] for i in trg_idx]

    return trg_tokens[1:-1]


def translate_Transformer_beam_search(src, vocabs, model, device, beam_size, max_decode_len):
    '''
    ...
    while len(completed_hypotheses) < beam_size and time_step < max_decode_len:
        ...
        trg_tensor = torch.tensor([[TRG_VOCAB.stoi[h] for h in hyp] for hyp in hypotheses], dtype=torch.long,
                                  device=device)
        ...
        for which_prev_hyp, next_hyp_id, best_score in zip(which_prev_hyps, next_hyp_ids, best_scores):
            ...
            next_hyp_word = TRG_VOCAB.itos[next_hyp_id]
...
def translate_LSTM(batch, vocabs, model, device, max_decode_len):
    '''
...
def translate_LSTM_beam_search(src, vocabs, model, device, beam_size, max_decode_len):
    '''
    biLSTM beam search
    '''
    TRG_VOCAB = vocabs['trg']
    ...
    while len(completed_hypotheses) < beam_size and time_step < max_decode_len:
        ...
        trg_tensor = torch.tensor([TRG_VOCAB.stoi[h[-1]] for h in hypotheses], dtype=torch.long, device=device)
...
def test(model, vocabs, device, test_iter, args, opt):
        ...
        if opt.model_type == 'Transformer':
            ...
            if args.beam_search:
                pred = translate_Transformer_beam_search(
                    batch.src, vocabs, model, device, args.beam_size, args.max_decode_len)
                pred = pred[0][0]  # top 1 prediction
            else:
                # Greedy search
                pred = translate_Transformer(batch, vocabs, model, device, args.max_decode_len)
        ...
        if opt.model_type == 'LSTM':
            ...
            if args.beam_search:
                pred = translate_LSTM_beam_search(
                    batch.src, vocabs, model, device, args.beam_search, args.max_decode_len)
                pred = pred[0][0]  # top prediction
            else:
                # Greedy search
                pred = translate_LSTM(batch, vocabs, model, device, args.max_decode_len)

        data = {}
        src = src[1:-1]
        data['src'] = ' '.join([vocabs['src'].itos[s] for s in src])
        data['trg'] = ' '.join([vocabs['trg'].itos[t] for t in trg])
...
def main(opt):
    ...
    (_, _, test_iter), vocabs = build_dataset(opt.model_type, test_args, device, model_args.vectors)
    model = build_model(model_args, opt.model_type, vocabs, device)
    model = model.to(device)
    test(model, vocabs, device, test_iter, test_args, opt)
    accuracy = calculate_result_accurary(test_args)
